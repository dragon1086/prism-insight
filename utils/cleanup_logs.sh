...
# 6. 패키지 매니저 캐시 (uv / pip)
# =============================================================================
# uv 캐시는 방치하면 계속 자란다. 2026-07-26 기준 6.4GB까지 불어나 디스크
# 사용률 83%의 주원인이었다. prune은 더 이상 참조되지 않는 항목만 제거하므로
# (전체 삭제인 clean과 달리) 다음 설치 속도에 미치는 영향이 거의 없다.
UV_BIN="${UV_BIN:-/root/.local/bin/uv}"
if [ -x "$UV_BIN" ]; then
    "$UV_BIN" cache prune >/dev/null 2>&1 \
        && echo "$(date): uv cache prune 완료" >> "$PROJECT_ROOT/utils/log_cleanup.log"
fi

# pip 캐시는 90일 이상 미접근 파일만 정리
PIP_CACHE="${HOME:-/root}/.cache/pip"
if [ -d "$PIP_CACHE" ]; then
    find "$PIP_CACHE" -type f -atime +90 -delete 2>/dev/null
fi
