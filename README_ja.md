...
## トレーディングシステムは失敗からどう学んだか

韓国市場の履歴には、エントリーを避けすぎることと、状態管理が不十分な
ままリスクを取ることという、相反する二つの失敗が現れました。
v1.16.7からv2.18までの改善により、システムはプロンプトのバイアス修正
から、相場局面・決済状態・再エントリーの決定論的な制御へ進化しました。

![観察バイアスから状態ベースのリスク制御へ進化したPRISM-INSIGHT](docs/images/trading-evolution-en.png)

> 数値は診断目的です。累積リターンは取引ごとのリターンの合計であり、
> 未エントリー候補の成績は事後観察値です。時間加重ポートフォリオ
> リターンや実現可能なバックテスト収益を示すものではありません。
