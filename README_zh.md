...
韩国市场的交易记录呈现了两种相反的失败：先是过度回避入场，随后又在
状态控制不足时承担风险。从 v1.16.7 到 v2.18，系统逐步从修正提示词偏差，
演进为对市场状态、退出状态和重新入场进行确定性控制。

![PRISM-INSIGHT 从观望偏差演进到基于状态的风险控制](docs/images/trading-evolution-en.png)

> 这些数字仅用于诊断。累计收益是逐笔交易收益率之和；未入场候选的结果
> 属于事后观察，并非时间加权的投资组合收益，也不是可实现的回测收益。
