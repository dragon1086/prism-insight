...
pytest(``test_sideways_downtrend_gate``)와 단독 실행
(``python tests/test_sideways_downtrend_gate.py``)을 모두 지원한다.
모듈 레벨에서 sys.exit를 호출하면 전체 pytest 수집이 중단되므로
검사 본문은 ``_run_checks()`` 안에 둔다.
"""
import os
import sys
import pandas as pd
...
def test_sideways_downtrend_gate():
    """pytest 진입점: 모든 검사가 통과해야 한다."""
    _run_checks()
    print(f"\n===== RESULT: {passed} passed, {failed} failed =====")
    assert failed == 0, f"{failed} gate checks failed"
