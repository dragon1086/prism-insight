...
def test_model_packet_preserves_scope_context_under_existing_byte_budget():
    row, child, main = fixture()
    blocks, gaps = call(row, child, main)
    state = {'sources': [{'source_id': 'fragment', 'url': child['url'], 'blocks': blocks}], 'gaps': gaps, 'calls': 0}
    result = packet('KR', '000000', '2026-09-20', state)
    notes = list(result['section_notes'].values())
    assert all(len(n.encode()) <= 6000 for n in notes)
    assert any('DART_EXPLICIT_VIEWER_TREE' in note for note in notes)
    assert any('조기상환' in note for note in notes)
    for note in notes:
        if 'DART_EXPLICIT_VIEWER_TREE' in note:
            payload = json.loads(note)
            for source in payload['sources']:
                assert source['url'] == child['url']
                assert source['source_id'] == 'fragment'
                original = next(b for b in blocks if b['excerpt'] == source['excerpt'])
                restored = expand_html_provenance(source['provenance'])
                restored.update(payload.get('source_provenance', {}).get('fragment', {}))
                assert restored == json.loads(json.dumps(original['provenance']))
