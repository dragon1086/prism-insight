...
def _strategy_proposal(strategy_id: StrategyId) -> TradePlanProposal:
    ...
    payload = {
        ...
        "bull_case": {
            "summary": f"{prefix} bullish path",
            "conditions": (f"{prefix} evidence remains supportive",),
            "confirmations": (f"{prefix} structure confirms",),
            "falsifiers": (f"{prefix} support fails",),
            "next_event": f"next {prefix} review",
            "valid_until": datetime(2026, 7, 25, 20, 0, tzinfo=timezone.utc),
            "evidence_ids": (f"{prefix}-evidence",),
        },
        "base_case": {
            "summary": f"{prefix} base path",
            "conditions": (f"{prefix} structure remains unchanged",),
            "confirmations": (f"{prefix} evidence persists",),
            "falsifiers": (f"{prefix} risk dominates",),
            "next_event": f"next {prefix} review",
            "valid_until": datetime(2026, 7, 25, 20, 0, tzinfo=timezone.utc),
            "evidence_ids": (f"{prefix}-evidence",),
        },
        "bear_case": {
            "summary": f"{prefix} bearish path",
            "conditions": (f"{prefix} risk evidence persists",),
            "confirmations": (f"{prefix} structure weakens",),
            "falsifiers": (f"{prefix} risk reverses",),
            "next_event": f"next {prefix} review",
            "valid_until": datetime(2026, 7, 25, 20, 0, tzinfo=timezone.utc),
            "evidence_ids": (f"{prefix}-risk",),
        },
        "entry_predicates": (),
        "stop_candidates": (),
        "target_candidates": (),
