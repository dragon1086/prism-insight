...
def test_exact_raw_section_keeps_conditions_and_original_locator():
    row, section = fixture()
    blocks, gaps = section_blocks(row, section)
    assert blocks and not gaps
    assert any('다만' in b['excerpt'] and '위반한 사실은 없습니다' in b['excerpt'] for b in blocks)
    for block in blocks:
        p = block['provenance']
        assert p['representation'] == 'DART_VIEWER_HTML'
        assert p['representation_sha256'] == section['sha256']
        assert p['source_path'].startswith('/')
        assert p['scope'] == 'consolidated'
        assert 'markdown_sha256' not in p
    state = {'sources': [{'source_id': 'D1', 'url': section['url'], 'blocks': blocks}], 'gaps': gaps, 'calls': 0}
    result = packet('KR', '327260', '2026-09-20', state)
    assert all(len(n.encode()) <= 6000 for n in result['section_notes'].values())
    assert any('다만' in n for n in result['section_notes'].values())
