...
Runs both as a pytest module (``test_issue_289_screening``) and standalone
(``python tests/test_issue_289_screening.py``). Module-level execution with
``sys.exit`` used to abort whole-suite pytest collection, so the checks live
inside ``_run_checks()``.
"""
import os
import sys
...
def test_issue_289_screening():
    """Pytest entry point: all checks must pass."""
    _run_checks()
    print(f"\n===== RESULT: {passed} passed, {failed} failed =====")
    assert failed == 0, f"{failed} screening checks failed"
