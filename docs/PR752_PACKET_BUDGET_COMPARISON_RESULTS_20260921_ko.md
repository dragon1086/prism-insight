...
**표781·1018의 필수 G1~G3를 자르지 않고 전달하는 경로를 오프라인 실험에서 확인했습니다.** 실제 후보 전체·순서·source/topic24개 제한·gold를 고정한 비교에서6,000과12,000은 실패했고,24,000과32,000은 packet 및 **명시적으로 같은 예산을 적용한 실험 agent instruction**에서 모두 통과했습니다.
...
2. **표현·실제 경쟁 비용이 남습니다.** 표781의 단독 admission trial은7,588바이트입니다.12,000 실험에서는 앞선 다른 주제 근거가 먼저 들어가 실제 trial이12,956바이트가 되어 거절됩니다. 두 필수 표만 강제로 넣은 공간 진단과 다릅니다.
...
기준은 기존 삼성생명 primary source와 원래 annual fragment를 사용한 component입니다. 원문 hash와 고정 gold를 대조했으며, 일반 후보 경쟁을 제거하지 않았습니다.
...
24,000/32,000에서 표781의83셀과 표1018의14셀을 원문 그대로 보존합니다. 승소·의무 소멸, 최선 추정치의 불확실성, 별개 자회사 소송 주체·청구금액·결과 불확실성을 기간·단위·출처와 함께 확인했습니다. gold 문서는 변경하지 않았고 실제 원문은 첨부/커밋하지 않습니다.
...
위24,000은 **비교한 네 값 중 이 사례가 통과한 첫 값**이지, 모든 기업에 필요한 최소값이나 운영 권고가 아닙니다. 예산이 늘면 앞선 후보의 포함 집합도 달라지므로 단순한 단독 표 크기로 전체 선정 성공을 예측하지 않습니다.
...
- 변경된 선정 결과가 오래된 cache로 대체되지 않도록 revision은 `bounded-html-v11-context-integrity`로 갱신했습니다. 새 의존성·원문 상한 확대·매매 규칙 변경은 없습니다.
...
- 최종 연구 입력 회귀 **2,246개 통과**, 기존 경고4개. 별도 provider 호환 통합 **46개 통과**. 새 미국 수집/실행을 의미하지 않습니다.
- 변경 Python10개 Ruff·AST/구문·diff 검사 통과. 독립 source-boundary 검토103개와 코드 리뷰71개는 전체 회귀와 중복되므로 합산하지 않습니다. 최종 코드 리뷰 APPROVE입니다. Python 전용 타입 검사 통과는 주장하지 않습니다.
