...
**quickReply가 렌더링되지 않았다.** 페이로드는 공식 규격과 일치
(`template.quickReplies[]`, `action`/`label`/`messageText`)하고 응답도 `HTTP 200`인데
채팅방에 버튼이 보이지 않았다.

원인 후보:

- `send_message`(선제 발송)에서는 미지원, `callback`(답장)에서만 지원
- 팀채팅방이 카드/버튼류를 미지원 (mention이 채널채팅 미지원, Carousel이 채널채팅 전용인
  것처럼 채팅방 유형별 제약이 실재한다)

ListCard + 항목 탭 + 카드버튼 조합도 발송했으나 탭 발화가 도착하지 않았다. 카드 자체가
렌더링됐는지 미확인.

### 사용 가능한 상호작용 수단 (문서 기준)

- `quickReplies` — 최대 10개, 렌더링 미확인
- `ListItem.action=message` + `messageText` — 리스트 항목 탭
- `Button.action=message` — 카드 버튼, 가로 2개 / 세로 3개, label 최대 14자
- **`Button.action=mention`** — **입력창에 봇 멘션을 자동 입력**. 그룹방 멘션 마찰을
  정면으로 해결하는 수단
- `Button.action=webLink` — PDF 링크 (설계 §10)
- `Button.action=invite` — 봇을 다른 채팅방에 초대 (확산, 심사기준 ③)
- 등록 명령어 `/v1/bot/commands/update` — 최대 20개, 상시 노출. 테스트 등록 성공
  (`리포트`/`평가`/`예측`/`순위`/`도움말`)
