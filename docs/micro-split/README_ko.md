...
2026-09-16 사용자 승인 후속: [오닐식 후보 감시 연계 설계](../ONEIL_WATCHLIST_MICRO_SPLIT_SHADOW_ko.md).
US 기술적 재검토 상태와 기존 적격 진입의 0→10% 투영을 별도 SHADOW 이벤트로 연결한다.
READY 자체를 BUY 승인으로 승격하지 않으며, 아래 기존 Phase 2a 표본·정책은 그대로 유지한다.
연계 구현을 연속 10→30→60→100% 캠페인이나 수익성 검증 완료로 표현하지 않는다.
