...
| 오닐식 후보 감시 × 초분할 연계 | **SHADOW (US, 승인된 설정)** | `trading/config/oneil_watchlist_shadow.json` + 기존 micro SHADOW 활성; `ONEIL_WATCHLIST_SHADOW_ENABLED=false`로 중단 | 정확한 연결·무영향 검사 + 완료 세션 20·연계 결정 30부터 재검토. 자동 LIVE 금지 | 완료 일봉 20일 고점 대용 감시. 이전 후보 재관측과 실제 BUY 적격 0→10 교집합만 연결. KR·장중 감시·연속 비중 단계·네 비교군 수익 검증은 후속. [설계](ONEIL_WATCHLIST_MICRO_SPLIT_SHADOW_ko.md) |
