...
> 2026-09-09 분석 계약 정정: 전략 원장은 실계좌와 독립입니다. 브로커 현금 부족이나
> 주문 거절 때문에 유효한 원장 진입·청산을 전략 평가에서 제외하지 않습니다.
> `ENTRY_QUALITY_DATA_ANALYSIS_HARNESS.md` v2에서 전략 성과와 브로커 집행 증거를
> 분리합니다. 이 분석 도구 정정은 새 매수 필터나 SHADOW/LIVE 승격이 아닙니다.
...
2026-09-16 사용자 승인으로 [US 후보 감시 × 초분할 연계 SHADOW](ONEIL_WATCHLIST_MICRO_SPLIT_SHADOW_ko.md)를
별도 사전등록 정책으로 시작한다. 기존 JSONL·shipper를 재사용하며, 기술적 READY와 실제
기존 BUY 적격 0→10% 투영의 정확한 교집합만 기록한다. 기존 CAPTURE/초분할 표본을
덮어쓰거나 최종 BUY·SELL·주문을 바꾸지 않는다. 현재 BUY의 즉시 진입/미진입 계약도
유지하며, 감시 후보를 LIVE로 다시 투입하는 기능과 연속 분할 캠페인은 아직 아니다.
