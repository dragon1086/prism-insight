...
- **US 보고서**: 공식 실적·공시·물가·금리 발표로 근거를 보강하고, 단위·계산을 코드로 검증하며,
  경쟁사 비교 표를 추가했습니다.
- **데이터·운영**: KIS 권리락 복구, 미국 보통주 전체 유니버스와 시총 10억 달러 선별, 완료 일봉 캐시,
  배치 실패 알림, 리포트 전용 OAuth 프록시를 정비했습니다.
...
- US 선별 대상을 상장 보통주 전체로 넓히고, 당일 거래대금 5천만 달러 이상 종목만 조회해 **시가총액 10억 달러
  이상**만 남깁니다(기본 켜짐,
...
9월 15일 이후 226개 커밋과 61개 PR을 묶었습니다. 이번에는 한국 종목 보고서가 가장 크게 바뀌었습니다.
...
· 미국은 상장 보통주 전체에서 시가총액 10억 달러 이상을 선별하고, 배치가 실패하면 알림을 보냅니다
...
This release groups 226 commits and 61 PRs since September 15. The biggest change is the Korean stock report.
...
· US screening covers all listed common stocks with a $1B market-cap floor, and failed batches now send alerts
