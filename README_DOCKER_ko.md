...
#### Python 패키지
- OpenAI API 및 OpenAI Agents (역할별 모델)
...
> **참고**: 기본 Docker cron은 미국 분석을 하루 2회 실행합니다. KST 기준 화-토는 미국 시간 기준 월-금에 해당합니다.
> Yahoo Finance 데이터는 15-20분 지연이 있어 이를 감안하여 스케줄을 설정했습니다.
> cron은 EST 기준이며 일광절약시간을 자동 보정하지 않습니다. EDT 기간(3~11월)에는 실행 시각을 다시 확인하세요.
