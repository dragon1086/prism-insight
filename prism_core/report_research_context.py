...
def apply_section_research(agent, section, prefetched, reference_date, language, *, source_budget_bytes=SOURCE_NOTE_BYTES):
    ...
    if isinstance(receipt, dict) and receipt.get('filing_parser') == 'material_v2':
        material_boundary = (
            ...
            'html_cell_grid_v1 retains the whole table: rows use zero-based row/column positions; '
            'strings (including empty strings) are original cell anchors, null is no original cell at that position, '
            'not zero. spans lists [row,col,rowspan,colspan] for merged cells; other anchors have span 1 by 1. '
