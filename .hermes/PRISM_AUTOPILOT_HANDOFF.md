...
## 2026-07-29 KR leadership-history and feedback-retrieval checkpoint

- Branch/worktree: `prism-insight/t_605b2273-kr-task-10-complete-leadership-history-a` / `.worktrees/t_605b2273`, reconciled onto exact current `origin/main` `7c5e6e4` after merged SHADOW_SCORE_V1 PR #53 and fail-soft StockEasy PR #54. This checkpoint implements the authoritative KR plan Task 10 foundation only; later mature market-outcome measurement, full real lesson formation/retrieval, final close-to-report UAT, and explicit user acceptance remain separate gates.
- Added append-only, PIT-bounded leadership history keyed by market/stable security/date/strategy/version, including candidate channels, relative strength, 52-week-high, momentum, peak, and `LEADING|EMERGING|NARROW|FADING` group state. Added strategy-scoped process-quality outcomes and guarded mature/PENDING market-outcome services; PENDING records retain `required_session_count`, `observed_session_count`, the exact official horizon maturity boundary in `next_eligible_session`, `NOT_YET_MEASURABLE`, and no fabricated return. Mature and partial-pending session lists must be the consecutive official KRX/NYSE sessions after day N, and the latest observed session must be closed at `observed_at`; calendar failure aborts before persistence.
- Query services read leadership/process/outcome history at an exact as-of boundary and retrieve candidate SHADOW lessons only by the complete market/security/strategy/version/regime/quant-score-policy-version key. The score-policy version is verified through proposal evidence bound to the immutable decision snapshot; passing it without the full candidate key now fails closed. Retrieved lessons expose immutable zero influence (`score_delta=0`, no policy/proposal effect); they cannot activate themselves or mutate scoring/risk.
- Genuine isolated KR evidence proof used the ignored `.hermes/uat/task10-20260729/research.sqlite` copy. Five real persisted 2026-07-29 proposals (3 `SWING_V1`, 2 `TREND_V1`) produced five process records and five explicit PENDING outcomes; decision-snapshot and proposal-table hashes were unchanged, observed sessions remained zero, and no score/risk or broker state changed. The source horizon has no later completed session yet, so 5/20-session market returns remain honestly deferred. The interrupted genuine product run did not retain enough authoritative group-state input to populate real leadership-history rows, and no mature SUPPORT/CONTRA lesson exists yet; leadership and exact SHADOW retrieval are therefore contract-tested foundations, not claimed as completed real operational feedback.
- Independent Claude Opus read-only review and current-tree follow-up returned PASS with no unresolved CRITICAL/HIGH/MEDIUM defect. Hermes accepted and closed the reviewer's silent score-version degradation finding through focused RED→GREEN, independently found and closed the civil-date/pre-close horizon defect through two focused RED→GREEN cycles, and obtained a follow-up PASS on the official-calendar and score-version changes. Remaining non-blocking notes are mature price-to-snapshot cross-validation, candidate-revision/security FK lineage, observation-only lesson binding, explicit pending→mature characterization, and the `next_eligible_session` terminal-boundary naming; none is promoted to operated readiness in this checkpoint.
- Definitive current-tree verification after the final rebase passed all 30 checked-in CI pytest invocations (`1,492` pass events and one intentional deselection), the focused app/feedback/storage slice (`74 passed`), full workflow-equivalent `compileall`, broker audit (`violations: 0`), `pip check`, CI YAML parsing, and `git diff --check`. After the first dashboard build correctly failed because dependencies were absent, the repository's documented `npm ci --legacy-peer-deps` installed the locked dependencies and `npm run build` passed; npm reported three existing high-severity advisories, and no dependency/lockfile edit was made. Delivery push, PR, exact-head CI, merge, and post-merge CI remain closeout gates.
- External effects through this checkpoint: GitHub fetch/prune; bounded real KIS market-data, public AgentNews KR, supplemental FMP, and existing ChatGPT OAuth model activity in the isolated genuine run; local SQLite writes only to the ignored Task 10 UAT copy; read-only Claude review; and npm registry reads for the local dashboard build. No user DB mutation, account/balance/holdings/broker/order/cancel/replace, external message, schedule/cron/launchd, credential change/output, destructive migration, live/risk change, push, PR, or merge has occurred. Local Task 10 commits exist and were rebased but are not yet pushed.

## 2026-07-29 KR user-oriented daily report/dashboard checkpoint

- Branch/worktree: `prism-insight/t_812f49b5-kr-task-9-render-user-oriented-kr-report` / `.worktrees/t_812f49b5`, reconciled by fast-forward to exact Task 8 merged `origin/main` `71587d1` before implementation. This checkpoint implements authoritative KR plan Task 9 only; scheduling/install/alerts remain Task 10 and no operated-readiness or user UAT acceptance is claimed yet.
- Added a formal Korean, concise-first nine-section SHADOW report projection: source/as-of/call/quality; KR market context; per-candidate symbol/channel/trigger plus paired SWING/TREND states and change label; strategy cards; conditional scenario controls; explicit gaps/conflicts and exact-level suppression; prior-run changes; next review; and collapsed audit internals. The dashboard exports the same ordered `research.kr_daily` layer additively under `prism_dashboard_v1` and renders it before legacy detail cards; missing call evidence, aggregate breadth/flows, and leadership-group context remain explicitly `UNAVAILABLE` rather than fabricated.
- Stale/PARTIAL/CONFLICT/UNAVAILABLE or non-ACCEPT quality suppresses exact price-bearing stop/target candidates and recursively removes price/entry-level/comparison/observed/bound values from both concise and audit projections. Invalid or incomplete proposals remain `INVALID_PROPOSAL` / `ANALYSIS_INCOMPLETE`; they are never invented as `NO_ENTRY`. The UAT seam now fails closed unless the leadership snapshot matches the dashboard and the report/dashboard stores agree exactly on each strategy's data snapshot, scenario state, completeness, and proposed decision.
- Strict RED→GREEN coverage now passes `25` focused report/UAT/dashboard tests, including ordered sections, candidate symbol/change, predicate-level suppression, same-snapshot verification, cross-store decision-drift rejection, page order, and no invented decision. A genuine read-only Task 8 KR persisted readback for `271560` projected the same leadership snapshot on report/dashboard, preserved both `SWING_V1` and `TREND_V1`, rendered four accumulated dashboard candidate/card pairs, kept report sections ordered, and returned `broker_called=false` / `schedule_activated=false`. SHA-256 values for all three source databases were identical before and after readback; no database was mutated.
- Independent Claude Code read-only review initially identified four MEDIUM concerns. Hermes accepted and fixed recursive predicate-level suppression, cross-store decision equality, report symbol/change columns, conditional re-entry/pyramiding/exit, incomplete data-gap projection, and page-order proof. The current-tree follow-up marked prior M1/M2/M3 resolved, M4 an accepted persisted-contract limitation, found no blocking HIGH/MEDIUM defect, and raised one non-blocking exact-one-match concern. Hermes rejected that concern against the genuine Task 8 database: every `(strategy_id, data_snapshot_id)` has exactly one security/proposal, and the current genuine multi-candidate readback passes the gate.
- Definitive frozen-tree hosted-CI-equivalent validation passes all 30 pytest command groups (`1,471` passing invocations plus one intentional deselection), full runtime/application `compileall`, broker-boundary audit (`violations: 0`), `pip check`, CI YAML parse, and `git diff --check`. The localhost Next.js dashboard production build also passes; its generated `next-env.d.ts` drift was removed before freezing the delivery manifest. The repository's required dependency installation remains `npm ci --legacy-peer-deps`; bare `npm ci` retains the pre-existing React 19/`vaul` peer conflict, and standalone `tsc --noEmit` retains many pre-existing errors in legacy unused components while reporting none for `app/page.tsx`. Commit/push/PR, exact-head CI, merge, post-merge verification, and Task 10 creation remain pending closeout.
- External effects through this checkpoint: GitHub fetch/prune, local dependency installation, private local readback outputs, and two read-only Claude review rounds. No network/provider/model call, account/balance/holdings/broker/order/cancel/replace, external message, schedule, credential mutation/output, destructive migration, commit, push, PR, or merge occurred in Task 9 so far.

## 2026-07-29 KR supplemental-leadership and safe StockEasy import checkpoint

- Branch/worktree: `prism-insight/t_8e58bad1-kr-task-7-add-supplemental-leadership-an` / `.worktrees/t_8e58bad1`, based on exact `origin/main` `9a39e74` (`HEAD...origin/main = 0/0` before delivery). This checkpoint implements authoritative KR plan Task 7 only; Task 8 daily-close composition/CLI, final user UAT, scheduling, and operated readiness are not claimed.
- Added strict immutable supplemental-leadership contracts and composition for generic market breadth, investor flows, leading groups, RS/highs, turnover, momentum, and peak-state observations. Supplemental nominations retain the `SUPPLEMENTAL_LEADERSHIP` channel, reconcile by the same stable KR `uuid5(NAMESPACE_URL, "prism:kr:{six_digit_symbol}")` identity as `CORE_PRISM`, preserve every unique candidate without truncation, and remain candidate-selection inputs only until the same KIS/official-evidence/SWING/TREND/validator path succeeds.
- Added a network/browser-free StockEasy sanitized snapshot import port. It requires separately recorded terms, permission basis, capture method, generic section, and exact source-scope approval; rejects unknown schema keys plus credential/cookie/token/internal-endpoint/account/payment/profile material in keys or high-risk string forms; bounds raw structure, collections, and temporary images; binds capture/export/import/as-of clocks and content/image hashes; rejects cross-security evidence binding; and deletes temporary images with existence verification. KIS/KRX authoritative values are immutable; disagreement creates visible `SUPPLEMENTAL_AUTHORITY_CONFLICT`, projects the effective supplement as `CONFLICT`, and forces supplemental scenario evidence to `REPORT_ONLY` without erasing KIS data.
- StockEasy terms/permission and an approved UI/export path were **not independently confirmed** in this run. No StockEasy page, export, screenshot, authenticated session, cookie, credential, payment/profile surface, or internal endpoint was accessed. The actual capability result is `UNAVAILABLE` / `APPROVED_UI_EXPORT_NOT_CONFIRMED`; fixture tests prove only the sanitized import contract, not live StockEasy verification. User-facing projection retains generic section identities while the vendor token remains internal provenance.
- Actual read-only KR core acceptance ran at `2026-07-29T15:56` KST for `214450` with benchmark `069500`. Two bounded KIS historical quotation calls returned HTTP `200`; AgentNews KR returned HTTP `200`; the persisted/read-back result was `REPORT_ONLY_READBACK_VERIFIED`, KIS remained the scenario identity/latest-bar authority, and missing supplemental fundamentals remained explicit. DART/KIND were capability-unavailable and not called; FMP returned HTTP `200` but no usable supplement. The StockEasy-unavailable port was evaluated separately and did not alter or erase the KIS result. Private `/tmp/prism-task7-live` databases/report were not staged and were deleted after verification.
- Strict TDD retained RED→GREEN evidence for exact permission/scope provenance, cross-security evidence rejection, symlink-safe image handling, value-side credential material rejection, and effective authority-conflict propagation. Current focused/neighboring proof is `48 passed`; all 30 hosted-CI-equivalent pytest command groups pass (`1,455` passing invocations plus one intentional deselection). `compileall`, `pip check`, broker-boundary audit (`violations: 0`), and `git diff --check` pass.
- Independent Claude read-only review initially returned `PASS` with two HIGH residual checks. Hermes verified exact stable-identity parity against the current KR candidate adapter, rejected the supplemental-candidate eligibility concern because eligibility is discovery-only and the scenario pack still requires KIS `MarketSnapshot`/features, accepted and fixed value-side secret-pattern screening through RED→GREEN, and adjudicated all MEDIUM/LOW findings. A complete current-tree follow-up returned `PASS`, confirmed no unresolved CRITICAL/HIGH defect, and left only fail-closed/documented residuals: pattern-based value screening is not omniscient, same-source display-name divergence rejects conservatively, JSON-shaped DAG over-rejection, internal provider presentation discipline, and trusted temporary-path caller scope.
- Delivery PR #50 (`https://github.com/mienne/prism-insight/pull/50`) contains the Task 7 implementation at code head `ad8ce8b`. Hosted run `30430027792` passed the complete fail-closed workflow on Python 3.10, 3.11, and 3.12. This handoff-only update requires its own exact-head green run before squash merge; no code, review, data-authority, or safety blocker remains.
- External effects through this checkpoint: GitHub fetch/push/PR/hosted CI, bounded actual KIS market-data-only reads, public AgentNews KR read, supplemental FMP read, local ChatGPT OAuth proxy lifecycle with zero structured model responses/tools, and two read-only Claude review rounds. No StockEasy live/UI/export access, DART/KIND call, account/balance/holdings/broker/order/cancel/replace, external message, schedule, credential mutation/output, destructive migration, direct/force push, or merge occurred yet.

## 2026-07-29 KR official-evidence cascade verified checkpoint

- Branch/worktree: `wt/t_7fef5f8c` / `.worktrees/t_7fef5f8c`, based on exact `origin/main` `931b78d` (`HEAD...origin/main = 0/0` after fetch/prune). This checkpoint implements authoritative KR plan Task 6 only; StockEasy remains Task 7 and no final KR daily-product/UAT or operated-readiness claim is made.
- Added a PIT DART contract/normalizer with injected transport, true official acceptance as `available_at`, stable receipt/hash/provider identity, bounded sanitized retries, future-filing filtering, and an explicit unavailable adapter because DART capability/credentials/terms were not independently approved for this live run. Added an explicit KIND/KRX port plus unavailable adapter rather than embedding or reverse-engineering an unapproved endpoint/export.
- Added the official-first KR evidence composer: DART → KIND/KRX → already-composed KIS market context → optional FMP normalization. DART evidence wins over FMP, same-vintage official conflicts and mapped suspension/management/delisting/unresolved-corporate-action risks veto deterministically, and any unrecognized official risk flag fails closed through the sanitized `UNRECOGNIZED_OFFICIAL_RISK_FLAG` veto. Missing FMP or missing official fundamentals no longer raises generic `LiveKREvidenceError`; it preserves technical/market features as `REPORT_ONLY`, suppresses strategy proposals/actionable levels, and records `MISSING_SUPPLEMENTAL_FUNDAMENTALS`. Missing core price/identity/session/basis/timing remains reject/`ANALYSIS_INCOMPLETE`. US retains mandatory core fundamentals.
- The live evidence/report provenance label now names only fundamental providers actually represented by normalized records. A no-FMP run no longer falsely renders `LIVE_KIS_FMP_AGENTNEWS_PIT`; DART-selected evidence renders DART rather than FMP. Product readback exposes quality reasons, missing/stale fields, and the sanitized cascade status without raw provider payloads or credential detail.
- Actual read-only acceptance for `214450` completed at `2026-07-29T14:41` KST through the current production `shadow-run` seam. Eight bounded KIS daily market-data calls returned HTTP `200`; the report recorded KIS `FRESH` at latest completed session `2026-07-28`. AgentNews KR returned HTTP `200`. FMP returned HTTP `200` but lacked usable two-statement PIT coverage; DART and KIND were explicitly capability-unavailable and were not called. The final persisted/read-back result was `REPORT_ONLY_READBACK_VERIFIED`, reason `partial_report_only:fundamental`, with technical SWING/TREND report-only feature snapshots retained, zero model calls/proposals, `broker_called=false`, and no generic live-evidence exception. Sanitized private artifacts are under ignored mode-0600 `.hermes/uat/task6-214450-current/`.
- Strict TDD retained decisive RED→GREEN evidence for no-FMP continuation, stale supplemental disposition, DART PIT/acceptance clocks, official precedence/conflict, unavailable adapters, KR-derived FMP symbol, US quality isolation, unknown official-risk fail-closed behavior, and truthful no-FMP provenance labels. Current focused/neighboring proof is `40 passed`; selected broad app/data/features/strategies/policy/LLM/feedback/e2e proof is `615 passed`; all 30 hosted-CI-equivalent pytest command groups pass (`1,428` passing invocations plus one intentional deselection). `compileall`, `pip check`, broker-boundary audit (`violations: 0`), and `git diff --check` pass. A bare root `pytest` remains unsuitable because legacy `tests/test_issue_289_screening.py` exits during collection; the exact hosted matrix avoids that pre-existing collector behavior. Added-line scanning found only the deliberate `fixture-key` test literal and no credential/private-key value, dangerous execution primitive, private artifact, or database path staged for delivery.
- Independent Claude Opus read-only review first BLOCKED on silent unknown official-risk flag dropping. Hermes accepted and fixed that HIGH with a focused RED→GREEN regression, independently verified hard-veto rejection in `ProposalValidator`, and corrected the separately discovered false FMP coverage label. A frozen current-tree follow-up returned `PASS`, confirmed both defects resolved, and found no new CRITICAL/HIGH issue. Its non-blocking residuals are dormant official-screening availability policy, pre-existing stale-AgentNews gate semantics, optional FMP call cost when DART already suffices, and official evidence-item audit completeness when FMP supplies fundamentals.
- External effects through this checkpoint: GitHub fetch/prune, actual KIS market-data-only reads, public AgentNews KR reads, supplemental FMP fundamentals reads, local ChatGPT OAuth proxy startup with zero model/tool calls, and read-only Claude review. No DART/KIND live call, account/balance/holdings/broker/order/cancel/replace, external message, schedule, credential mutation/output, destructive migration, commit, push, PR, or merge occurred yet.
