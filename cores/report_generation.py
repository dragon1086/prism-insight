...
3. **The story behind the numbers** — 2-3 bullets, each in the pattern "On the surface …, but the filings show … → so for investors this means …". For KR stocks use the key points and body of the 'DART 주요 재무·사업 위험 분석' (DART filing-risk) chapter; for US stocks use official results/guidance and financial-quality facts. Pick the non-obvious facts, e.g., profit boosted by one-off tax income; debt that fell because a convertible bond turned into shares (less repayment risk, but more shares and dilution); receivables or inventory growing faster than sales; clustered debt maturities or assets pledged as collateral; large related-party transactions. Explain any accounting term in plain words in parentheses the first time (e.g., "convertible bond (a bond that can later be exchanged for shares)"). Use rounded units ($B/$M, or 100-million/trillion won) rather than full-digit amounts. If filing-depth evidence is absent, use only facts from the company status/financial chapters. Always judge an apparent burden net of what offsets it (e.g., debt coming due vs several times as much cash and short-term financial assets means 'maturity exists but repayment capacity is ample'); never imply risk from gross amounts alone, and actively surface cases that look bad on the surface but are fine underneath.
