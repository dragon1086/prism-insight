...
REASONS = frozenset({'profit_attribution', 'comparison_basis', 'availability_scope'})
...
def _validate_and_apply(reports, payload):
    ...
    for edit in edits:
        ...
        if section not in EDITABLE_SECTIONS or section not in reports or edit['reason'] not in REASONS:
...
async def edit_and_summarize(section_reports, company_name, company_code, reference_date, language='ko'):
    ...
    instruction = (
        ...
        '수정은 다음 세 종류만 허용합니다: profit_attribution: 영업이익 아래의 관계기업 투자손익을 '
        ...
        '허용 섹션: ' + ', '.join(sorted(EDITABLE_SECTIONS)) + '. 그 외 섹션은 읽기 전용입니다.\n'
        ...
        'comparison_basis 또는 availability_scope"}], "unresolved":[]}.'
    )
    if language != 'ko':
    ...
    patched, summary = _validate_and_apply(section_reports, payload)
    usage = result.usage
