...
- 현재가(CP, USD), 발행주식수, 최신 TTM 매출, 최신 TTM 영업이익
- 과거 5년 매출 CAGR, 과거 5년 영업이익률 범위

다음을 LLM이 직접 계산합니다(2단계 근사):
- 현재 시가총액(EV)이 정당화하는 향후 10년 매출 CAGR (영구성장률 2.5%, 할인율 8% 가정 — 미국 주식 기준)
...
예시:
  "P/E 18.5x [actual] vs 섹터 중앙값 22x [actual] → 16% 할인 [inference]"
  "추정 10년 CAGR 25% [inference] vs 과거 5년 CAGR 11% [actual] → stretched"
  "FY26 예상 영업이익률 14% [assumption] (가이던스 미공개)"
...
- Current price (CP, USD), shares outstanding, latest TTM revenue, latest TTM operating income.
...
- Implied 10-year revenue CAGR that the current EV/market cap would justify
  assuming terminal growth = 2.5% and discount rate = 8% (US equity).
...
Examples:
  "P/E 18.5x [actual] vs sector median 22x [actual] → 16% discount [inference]"
  "implied 10y CAGR 25% [inference] vs past-5y CAGR 11% [actual] → stretched"
  "FY26 op margin 14% [assumption] (no guidance disclosed)"
